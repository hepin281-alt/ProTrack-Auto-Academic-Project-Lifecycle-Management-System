...
export const AppShell: React.FC<AppShellProps> = ({ children, currentPage }) => {
 ...
 const [unreadCount, setUnreadCount] = useState(0);
  ...
  useEffect(() => {
    ...
    const fetchCounts = async () => {
      try {
        const annRes = await api.getUnreadAnnouncementCount(token).catch(() => ({ count: 0 }));
        let notifRes = { count: 0 };
        if (user?.role === 'STUDENT') {
          notifRes = await api.getUnreadNotificationsCount(token).catch(() => ({ count: 0 }));
        }
        setUnreadCount((annRes.count || 0) + (notifRes.count || 0));
      } catch (error) {
        console.error(error);
      }
    };
 ...
 >
 {/* Glass sidebar panel */}
 <div className="flex flex-col h-full bg-white/5/[0.05] backdrop-blur-xl border-r border-white/[0.08]">
 ...
 className="p-2 text-white/60 hover:text-white hover:bg-white/5/10 rounded-lg transition-all"
 >
 ...
 isActive
 ? 'bg-white/5/15 text-white shadow-lg shadow-white/5 border border-white/20'
 : 'text-white/60 hover:text-white hover:bg-white/5/10'
 }`}
 >
 <div className={`p-1.5 rounded-lg transition-all ${
 isActive
 ? `bg-gradient-to-br ${roleGradient} shadow-lg`
 : 'bg-white/5/5 group-hover:bg-white/5/10'
 }`}>
 ...
 <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-white/5/5 border border-white/10">
 ...
 {/* Top Navbar */}
 <header className="h-16 flex items-center justify-between px-6 border-b border-white/[0.08] bg-white/5/[0.03] backdrop-blur-md flex-shrink-0">
 ...
 type="text"
 placeholder="Search..."
 className="w-full pl-9 pr-4 py-2 bg-white/5/5 border border-white/10 text-white placeholder:text-white/30 rounded-xl text-sm focus:outline-none focus:border-white/30 focus:bg-white/5/8 transition-all"
 />
 </div>
 </div>
 ...
 className="relative p-2 text-white/50 hover:text-white hover:bg-white/5/10 rounded-xl transition-all"
 >
 <Bell size={18} />
 {unreadCount > 0 && (
 <span className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] flex items-center justify-center bg-gradient-to-br from-red-400 to-pink-500 rounded-full text-[9px] font-bold text-white shadow-lg px-1">
 ...
 className="flex items-center gap-2.5 px-3 py-2 rounded-xl bg-white/5/5 border border-white/10 hover:bg-white/5/10 hover:border-white/20 transition-all"
 >

...
const ROLES = [
 ...
 label: 'Coordinator',
 description: 'Manage groups, allocate guides & oversee the department',
 icon: Clipboard,
...
const FEATURES = [
 ...
 desc: 'Auto-match guides to groups based on workload, expertise tags, and domain similarity using ML scoring.',
 color: 'from-purple-500 to-pink-500',
 stat: 'ML-powered matching',
 },
 {
 icon: ShieldCheck,
...
const STATS = [
 { value: 1200, suffix: '+', label: 'Students', icon: GraduationCap, color: 'text-blue-400' },
 { value: 340, suffix: '+', label: 'Projects Tracked', icon: Layers, color: 'text-purple-400' },
 { value: 48, suffix: '', label: 'Faculty Guides', icon: Users, color: 'text-pink-400' },
 { value: 99, suffix: '.8%', label: 'Uptime', icon: Zap, color: 'text-emerald-400' },
];
...
const MockDashboard: React.FC = () => (
 <div className="relative bg-white/5/[0.03] border border-white/[0.08] rounded-3xl p-6 backdrop-blur-2xl overflow-hidden shadow-2xl">
 ...
 <div key={item.label} className="bg-white/5/[0.04] border border-white/[0.06] rounded-xl p-3 text-center">
 ...
 <div className="w-full h-2 bg-white/5/[0.06] rounded-full overflow-hidden">
 <div className="h-full w-[52%] bg-gradient-to-r from-purple-500 to-blue-500 rounded-full" />
 </div>
 </div>
 ...
 { action: 'Proposal approved by Coordinator', time: '3d ago', dot: 'bg-purple-400' },
 ].map((item, i) => (
...
const LandingPage: React.FC = () => {
 ...
 const [claimExpertise, setClaimExpertise] = useState('');
 ...
 if (selectedRole === 'GUIDE') payload.expertise_tags = claimExpertise.split(',').map((t: string) => t.trim()).filter(Boolean);
 ...
 return (
 <div className="bg-[#0a0015] text-white min-h-screen overflow-x-hidden" style={{ fontFamily:"'Inter', system-ui, sans-serif" }}>

 {/* ══════════════════════ NAVBAR ══════════════════════ */}
 <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-500 ${scrolled ? 'bg-[#0a0015]/85 backdrop-blur-2xl border-b border-white/[0.06]' : ''}`}>
 ...
 <button onClick={scrollToPortal} className="group relative px-6 py-2.5 rounded-xl font-bold text-sm overflow-hidden transition-all duration-200 hover:scale-105 active:scale-95 text-white">
 ...
 <h1 className="animate-slide-up delay-100 text-5xl md:text-6xl lg:text-7xl font-black leading-[1.08] tracking-tight mb-6 text-white">
 Academic Projects.{' '}
 <br />
 ...
 <button onClick={scrollToPortal} className="group relative inline-flex items-center justify-center gap-2 px-8 py-4 rounded-2xl font-bold text-base text-white overflow-hidden transition-all duration-200 hover:scale-105 active:scale-95 shadow-[0_0_40px_rgba(168,85,247,0.4)]">
 <div className="absolute inset-0 bg-gradient-to-r from-purple-600 via-blue-600 to-purple-600 bg-[length:200%_100%]" />
 <span className="relative">Access Portal</span>
 <ArrowRight className="w-5 h-5 relative group-hover:translate-x-1 transition-transform" />
 </button>
 <button onClick={() => scrollTo('features')} className="inline-flex items-center justify-center gap-2 px-8 py-4 rounded-2xl font-bold text-base text-white glass hover:bg-white/5/[0.07] transition-all duration-200 hover:scale-105 active:scale-95">
 Explore Features
 ...
 {/* Stats row */}
 <div ref={statsRef} className="animate-slide-up delay-400 flex flex-wrap gap-6 mt-12 pt-8 border-t border-white/[0.06]">
 {STATS.map((stat, i) => {
 const count = useCountUp(stat.value, 1800, statsVisible);
 return (
 <div key={i} className="text-center">
 <p className={`text-2xl font-black ${stat.color}`}>{count}{stat.suffix}</p>
 ...
 <div className="w-1.5 h-1.5 rounded-full bg-white/5/40 animate-bounce" />
 </div>
 </button>
 </section>
 ...
 <div key={idx} className="group relative glass rounded-3xl p-7 hover:bg-white/5/[0.06] transition-all duration-300 hover:-translate-y-1 cursor-default overflow-hidden">
 ...
 <div key={idx} className="group glass rounded-3xl p-7 hover:bg-white/5/[0.06] transition-all duration-300 hover:-translate-y-1 flex items-start gap-5">
 ...
 {/* ══════════════════════ HOW IT WORKS ══════════════════════ */}
 <section id="how-it-works" className="relative py-28 px-6 border-y border-white/[0.04] bg-white/5/[0.01]">
 <div className="max-w-7xl mx-auto">
 <div className="text-center mb-20">
 <p className="text-xs font-bold uppercase tracking-[0.3em] text-blue-400 mb-4">Project Lifecycle</p>
 <h2 className="text-4xl md:text-5xl font-black mb-5 text-white">Built for every step of the <span className="shimmer-text">academic year.</span></h2>
 ...
 {[
 { num: '01', title: 'Planning', desc: 'Form your group, submit your project proposal, and get it approved by the Coordinator.', color: 'from-blue-500 to-cyan-500', tags: ['Group Formation', 'Proposal'] },
 ...
 { num: '04', title: 'Submission', desc: 'Present your completed project to the committee for rubric-based evaluation and final score.', color: 'from-emerald-500 to-teal-500', tags: ['Evaluation', 'Final Score'] },
 ].map((phase, idx) => (
 ...
 {/* ══════════════════════ PORTAL / LOGIN ══════════════════════ */}
 <section id="portal" ref={portalRef} className="relative py-28 px-6 bg-white/5/[0.01] border-t border-white/[0.04] overflow-hidden">
 <div className="absolute inset-0 pointer-events-none">
 ...
 <h2 className="text-4xl md:text-5xl font-black mb-5 text-white">Access your <span className="shimmer-text">workspace.</span></h2>
 ...
 className="group relative glass-strong rounded-3xl p-7 text-left hover:bg-white/5/[0.08] transition-all duration-300 hover:-translate-y-2 overflow-hidden cursor-pointer"
 ...
 className="w-full pl-11 pr-4 py-3.5 bg-white/5/[0.04] border border-white/[0.10] text-white rounded-xl focus:outline-none focus:border-purple-500/60 focus:bg-white/5/[0.07] transition-all text-sm" />
 </div>
 </div>
 ...
 className="w-full pl-11 pr-12 py-3.5 bg-white/5/[0.04] border border-white/[0.10] text-white rounded-xl focus:outline-none focus:border-purple-500/60 focus:bg-white/5/[0.07] transition-all text-sm" />
 ...
 className={`w-full py-4 rounded-xl font-bold text-white flex items-center justify-center gap-2 transition-all ${isLoading ? 'bg-white/5/10 cursor-not-allowed text-white/50' : `bg-gradient-to-r ${activeRole?.color} hover:opacity-90 hover:shadow-xl active:scale-[0.98]`}`}>
 ...
 {selectedRole === 'GUIDE' && (
 <div>
 <label className="block text-xs font-bold text-white/40 uppercase tracking-widest mb-2">Expertise Domains</label>
 <input type="text" value={claimExpertise} onChange={e => setClaimExpertise(e.target.value)} placeholder="Machine Learning, IoT, Web Dev..."
 className="w-full px-4 py-3.5 bg-white/5/[0.04] border border-white/[0.10] text-white rounded-xl focus:outline-none focus:border-purple-500/60 focus:bg-white/5/[0.07] transition-all text-sm" />
 </div>
 )}
 <div>
 ...
 className={`w-full py-4 mt-2 rounded-xl font-bold text-white flex items-center justify-center gap-2 transition-all ${claimLoading ? 'bg-white/5/10 cursor-not-allowed text-white/50' : `bg-gradient-to-r ${activeRole?.color} hover:opacity-90 active:scale-[0.98]`}`}>
 ...
 <span className="text-lg font-black tracking-tight text-white">ProTrack<span className="text-white/40">-Auto</span></span>
 </div>
 ...
 { label: 'Login Portal', id: 'portal' },
 ].map(link => (
 <li key={link.id}>
 <button onClick={() => scrollTo(link.id)} className="text-sm text-white/50 hover:text-white transition-colors">{link.label}</button>
 </li>
 ))}
 </ul>
 </div>
 ...
 <li className="text-sm text-white/50">🕘 Mon–Fri, 9am–5pm IST</li>
 </ul>
 </div>
 </div>

...
export const CoordinatorUsers: React.FC = () => {
 ...
 // Pagination & search for student whitelist
 ...
 const filteredWhitelist = whitelist.filter(s =>
 !studentSearch ||
 s.full_name?.toLowerCase().includes(studentSearch.toLowerCase()) ||
 s.email?.toLowerCase().includes(studentSearch.toLowerCase()) ||
 s.prn_no?.toLowerCase().includes(studentSearch.toLowerCase())
 );
 const totalPages = Math.ceil(filteredWhitelist.length / ITEMS_PER_PAGE);
 const paginatedWhitelist = filteredWhitelist.slice((studentPage - 1) * ITEMS_PER_PAGE, studentPage * ITEMS_PER_PAGE);
 ...
 <p className="text-white/50 text-sm ml-11">Manage student whitelists and faculty accounts</p>
 </div>
 </div>
 ...
 <div>
 <label className="block text-xs font-bold text-white/50 uppercase tracking-widest mb-2">PRN No</label>
 <input 
 type="text" 
 required
 ...
 className="w-full px-4 py-3 bg-white/5 border border-white/[0.08] text-white placeholder:text-white/25 rounded-xl text-sm focus:outline-none focus:border-white/[0.08] transition-all"
 />
 </div>
 <div>
 ...
 <h3 className="text-xs font-bold text-white/50 uppercase tracking-widest mb-4">Bulk Upload Students</h3>
 ...
 <p className="text-[10px] text-white/50 mt-2">File must contain columns: prn_no, email, full_name</p>
 )}
 </div>
 <button
 ...
 <div className="text-sm font-bold text-white/50 bg-white/5 px-3 py-1.5 rounded-lg border border-white/[0.08]">
 <span className="text-emerald-400">{claimedCount}</span> claimed / {whitelist.length} total
 </div>
 <button 
 onClick={handleExportCSV}
 className="text-xs font-bold bg-white/[0.04] /10 hover:bg-white/5/20 text-white px-3 py-2 rounded-lg transition-colors border border-white/[0.08]"
 >
 Export CSV
 </button>
 </div>
 ...
 className="w-full px-4 py-2.5 bg-white/5 border border-white/[0.08] text-white placeholder:text-white/25 rounded-xl text-sm focus:outline-none focus:border-white/[0.08] transition-all"
 />
 </div>
 ...
 <thead>
 <tr className="border-b border-white/[0.08] text-[10px] uppercase tracking-widest text-white/50 bg-white/5">
 ...
 <tr>
 <td colSpan={5} className="py-8 text-center text-white/50 text-xs italic">
 {studentSearch ? 'No students match your search.' : 'No students whitelisted yet.'}
 </td>
 </tr>
 ) : (
 paginatedWhitelist.map(student => (
 <tr key={student.id} className="border-b border-white/[0.08][0.04] hover:bg-white/[0.04] transition-colors">
 <td className="py-3 px-4 font-mono text-xs">{student.prn_no}</td>
 <td className="py-3 px-4">{student.full_name}</td>
 <td className="py-3 px-4 text-xs text-white/50">{student.email}</td>
 ...
 Showing {(studentPage - 1) * ITEMS_PER_PAGE + 1}–{Math.min(studentPage * ITEMS_PER_PAGE, filteredWhitelist.length)} of {filteredWhitelist.length}
 </p>
 <div className="flex gap-1">
 <button
 onClick={() => setStudentPage(p => Math.max(1, p - 1))}
 disabled={studentPage === 1}
 className="px-3 py-1.5 text-xs rounded-lg bg-white/5 border border-white/[0.08] text-white/50 hover:text-white hover:bg-white/[0.04] /10 disabled:opacity-30 transition-all"
 >← Prev</button>
 {Array.from({ length: totalPages }, (_, i) => i + 1)
 .filter(p => p === 1 || p === totalPages || Math.abs(p - studentPage) <= 1)
 .map((p, idx, arr) => (
 <React.Fragment key={p}>
 {idx > 0 && arr[idx - 1] !== p - 1 && <span className="px-2 text-white/50 text-xs flex items-center">…</span>}
 <button
 onClick={() => setStudentPage(p)}
 className={`px-3 py-1.5 text-xs rounded-lg border transition-all ${
 p === studentPage
 ? 'bg-orange-500 border-orange-500 text-white font-bold'
 : 'bg-white/5 border-white/[0.08] text-white/50 hover:text-white hover:bg-white/[0.04] /10'
 }`}
 >{p}</button>
 </React.Fragment>
 ))
 ...
 <p className="text-xs text-white/50">Manually provision Faculty Guides or Committee members</p>
 </div>
 </div>

 <form onSubmit={handleRegisterFaculty} className="space-y-4">
 <div>
 <label className="block text-xs font-bold text-white/50 uppercase tracking-widest mb-2">Role</label>
 <select 
 value={registerRole}
 onChange={e => setRegisterRole(e.target.value)}
 className="w-full px-4 py-3 bg-white/5 border border-white/[0.08] text-white rounded-xl text-sm focus:outline-none focus:border-white/[0.08] transition-all appearance-none"
 >
 ...
 {/* Faculty Bulk Upload */}
 <div className="mt-8 pt-6 border-t border-white/[0.08]">
 <h3 className="text-xs font-bold text-white/50 uppercase tracking-widest mb-4">Bulk Upload Faculty</h3>
 <div className="space-y-4">
 <div>
 <input 
 type="file" 
 ...
 <p className="text-[10px] text-white/50 mt-2">File must contain columns: email, employee_id, full_name, role</p>
 )}
 </div>
 <button
 ...
 <tr>
 <td colSpan={4} className="py-8 text-center text-white/50 text-xs italic">
 No pending invitations.
 </td>
 </tr>
 ) : (
 facultyWhitelist.filter(f => !f.is_claimed).map(faculty => (
 <tr key={faculty.id} className="border-b border-white/[0.08][0.04] hover:bg-white/[0.04] transition-colors">
 <td className="py-3 px-4">{faculty.full_name}</td>
 <td className="py-3 px-4 text-xs text-white/50">{faculty.email}</td>
 <td className="py-3 px-4">
 ...
 <tr key={faculty.faculty_id} className="border-b border-white/[0.08][0.04] hover:bg-white/[0.04] transition-colors">
 <td className="py-3 px-4">
 <div className="font-semibold text-white">{faculty.full_name || 'Un-setup Profile'}</div>
 <div className="text-xs text-white/50">{faculty.email}</div>
 </td>
 <td className="py-3 px-4">
 ...
 <td className="py-3 px-4">
 {faculty.role === 'GUIDE' ? (
 <span className="text-white/50 font-mono text-xs bg-white/5 px-2 py-1 rounded">
 ...
 className="w-16 px-2 py-1 bg-white/5 border border-white/[0.08] rounded text-white text-xs text-center"
 autoFocus
 />
 <button 
 ...
 disabled={isSavingWorkload}
 className="p-1 rounded bg-white/[0.04] /10 text-white/50 hover:bg-white/5/20 transition-colors"
 >
 <X size={14} />
 </button>
 </div>
 ...
 className="text-white/50 hover:text-white font-mono text-xs bg-white/5 hover:bg-blue-500/20 hover:text-blue-300 transition-all border border-transparent hover:border-blue-500/30 px-3 py-1 rounded cursor-pointer"
 ...
 <p className="text-xs text-white/50">Students registered but not in any group</p>
 </div>
 </div>
 {orphans.length > 0 && (
 <button 
 ...
 <th className="py-3 px-4 font-semibold">PRN No</th>
 <th className="py-3 px-4 font-semibold">Email</th>
 <th className="py-3 px-4 font-semibold">Batch Year</th>
 ...
 {isLoadingOrphans ? (
 <tr>
 <td colSpan={3} className="py-8 text-center text-white/50 text-xs">
 <RefreshCw size={16} className="animate-spin inline mr-2" /> Loading...
 </td>
 </tr>
 ) : orphans.length === 0 ? (
 ...
 orphans.map(student => (
 <tr key={student.student_id} className="border-b border-white/[0.08][0.04] hover:bg-white/[0.04]">
 <td className="py-3 px-4 font-mono text-xs">{student.prn_no}</td>
 <td className="py-3 px-4 text-xs text-white/50">{student.email}</td>
 <td className="py-3 px-4 text-xs text-white/50">{student.batch_year || 'N/A'}</td>
 <td className="py-3 px-4 text-xs text-white/50">{new Date(student.created_at).toLocaleDateString()}</td>
 </tr>
 ))
 )}
 </tbody>

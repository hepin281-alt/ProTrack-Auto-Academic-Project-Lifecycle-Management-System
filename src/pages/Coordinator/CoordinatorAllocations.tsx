...
function ScoreBar({ label, value, max, color }: { label: string; value: number; max: number; color: string }) {
 ...
 <span className="text-white/50">{value.toFixed(1)} / {max}</span>
 </div>
...
function ActionBadge({ action }: { action: string }) {
 ...
 return (
 <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${map[action] || 'bg-white/[0.04] /10 text-white/50'}`}>
 {action.replace('_', ' ')}
 </span>
 );
}
...
export const CoordinatorAllocations: React.FC = () => {
 ...
 queryKey: ['faculty'],
 queryFn: () => api.getFacultyList(token!),
 enabled: !!token,
 });
 ...
 >
 <div className="absolute inset-0 bg-white/5/20 hover:bg-transparent transition-colors duration-300 pointer-events-none" />
 ...
 <button onClick={() => setBatchResults(null)} className="text-white/50 hover:text-white">
 <X size={14} />
 </button>
 </div>
 ...
 <p className="text-[10px] text-white/50 uppercase font-semibold">{s.replace('_', ' ')}</p>
 </div>
 ))}
 </div>
 </div>
 ...
 {/* Tabs */}
 <div className="flex gap-1 mb-6 p-1 bg-white/[0.04] rounded-xl border border-white/[0.08][0.06] w-fit">
 {tabs.map(t => (
 <button
 key={t.id}
 ...
 activeTab === t.id
 ? 'bg-white/[0.04] /10 text-white shadow'
 ...
 <div className="p-4 border-b border-white/[0.08][0.06] bg-white/[0.04] flex justify-between items-center">
 ...
 <p className="text-[10px] text-white/50 mt-1 italic">No proposal submitted yet</p>
 )}
 </button>
 </motion.div>
 ))}
 ...
 isTop ? 'border-indigo-500/40 bg-indigo-500/5 hover:border-indigo-500/60' : 'border-white/[0.08][0.06] bg-white/[0.04] hover:border-white/[0.08][0.15]'
 )}
 >
 {/* Guide Header */}
 <div className="p-4">
 ...
 <p className="text-[10px] text-white/50">{guide.email}</p>
 </div>
 ...
 <span key={tag} className="px-1.5 py-0.5 bg-white/5 border border-white/[0.08] rounded text-[9px] text-white/50">
 {tag}
 </span>
 ))}
 <span className="px-1.5 py-0.5 bg-white/5 border border-white/[0.08] rounded text-[9px] text-white/50">
 ...
 className="p-2 rounded-lg bg-white/5 hover:bg-white/[0.04] /10 text-white/50 hover:text-white transition-all"
 >
 ...
 {isExpanded && (
 <div className="px-4 pb-4 pt-0 border-t border-white/[0.08][0.06] space-y-2">
 <p className="text-[10px] font-bold text-white/50 uppercase tracking-widest mb-3 pt-3">Score Breakdown</p>
 ...
 className="w-full px-3 py-2 text-xs bg-white/5 border border-white/[0.08] rounded-lg text-white placeholder-white/20 focus:outline-none focus:border-white/[0.08]"
 />
 </div>
 </div>
 )}
 ...
 className="p-1.5 rounded-lg bg-white/5 hover:bg-white/[0.04] /10 text-white/50 hover:text-white transition-all">
 <RefreshCw size={12} />
 </button>
 </div>
 <div className="overflow-x-auto">
 {loadingAudit ? (
 <div className="flex items-center justify-center py-12"><Loader2 size={24} className="animate-spin text-white/50" /></div>
 ) : auditLogs.length === 0 ? (
 <div className="text-center py-12 text-white/50 text-sm">No allocation events recorded yet.</div>
 ) : (
 <table className="w-full text-sm">
 <thead>
 <tr className="border-b border-white/[0.08][0.06]">
 {['Timestamp', 'Action', 'Group', 'Guide', 'Score', 'By', 'Notes'].map(h => (
 <th key={h} className="text-left px-4 py-3 text-[10px] font-bold text-white/50 uppercase tracking-wider">{h}</th>
 ))}
 </tr>
 </thead>
 <tbody>
 {auditLogs.map(log => (
 <tr key={log.audit_id} className="border-b border-white/[0.08][0.04] hover:bg-white/[0.04] transition-colors">
 ...
 <td className="px-4 py-3 text-xs text-white/50">{log.guide_name || '—'}</td>
 ...
 <td className="px-4 py-3 text-[11px] text-white/50">{log.performed_by_name || '—'}</td>
 <td className="px-4 py-3 text-[11px] text-white/50 max-w-[200px] truncate">{log.notes || '—'}</td>
 </tr>
 ))}
 </tbody>
 </table>
 ...
 className="w-full px-3 py-2 text-sm bg-white/5 border border-white/[0.08] rounded-xl text-white focus:outline-none focus:border-white/[0.08] appearance-none pr-10"
 >
 <option value="" disabled>Select a Guide...</option>
 {(Array.isArray(facultyList) ? facultyList : []).filter((f: any) => f.role === 'GUIDE' || f.role === 'COMMITTEE').map((faculty: any) => (
 <option key={faculty.faculty_id} value={faculty.faculty_id} className="bg-[#09090b]">
 ...
 className={cn('w-8 h-8 rounded-lg text-sm font-bold transition-all', ratingValue >= n ? 'bg-amber-500 text-white ' : 'bg-white/5 text-white/50')}>
 {n}
 </button>
 ))}
 </div>
 ...
 className="px-3 py-2 text-sm bg-white/5 border border-white/[0.08] rounded-xl text-white placeholder-white/30 focus:outline-none focus:border-white/[0.08]"
 />
 <button
 ...
 {/* Ratings Table */}
 <div className="rounded-2xl bg-white/[0.04] border border-white/[0.08] overflow-hidden">
 <div className="p-4 border-b border-white/[0.08][0.06]">
 <h3 className="text-xs font-bold text-white/50 uppercase tracking-widest">Rating History</h3>
 </div>
 {ratings.length === 0 ? (
 <div className="text-center py-12 text-white/50 text-sm">No ratings submitted yet.</div>
 ) : (
 <table className="w-full text-sm">
 <thead>
 <tr className="border-b border-white/[0.08][0.06]">
 ...
 <tbody>
 {ratings.map((r: any) => (
 <tr key={r.rating_id} className="border-b border-white/[0.08][0.04] hover:bg-white/[0.04]">
 ...
 <td className="px-4 py-3 text-[11px] text-white/50 max-w-[200px] truncate">{r.comments || '—'}</td>
 <td className="px-4 py-3 text-[11px] text-white/50">{r.academic_year}</td>
 <td className="px-4 py-3 text-[11px] text-white/50">{new Date(r.created_at).toLocaleDateString()}</td>
 </tr>
 ))}
 </tbody>
 </table>

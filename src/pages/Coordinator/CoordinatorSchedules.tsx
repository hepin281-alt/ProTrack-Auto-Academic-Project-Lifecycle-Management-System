...
export const CoordinatorSchedules: React.FC = () => {
 ...
 const years = fetchedYears.years && fetchedYears.years.length > 0 ? fetchedYears.years : [];
 setAvailableBatchYears(years);
 
 if (years.length > 0 && (!selectedYear || !years.includes(parseInt(selectedYear)))) {
     setSelectedYear(years[0].toString());
 }
 if (years.length > 0 && !years.includes(selectedBatchYear)) {
 ...
 <p className="text-white/50 text-sm ml-11">Assign presentation slots and manage overarching project deadlines</p>
 </div>
 ...
 onChange={(e) => {
     ...
     setSelectedBatchYear(parseInt(e.target.value, 10));
 }}
 className="ml-3 px-2 py-1 bg-white/5 border border-white/[0.08] text-white rounded text-xs focus:outline-none"
 >
 ...
 className="w-full px-3 py-2 bg-white/5 border border-white/[0.08] text-white rounded-lg text-sm focus:outline-none focus:border-emerald-500/50"
 />
 </div>
 );
 })}
 ...
 className="w-full px-3 py-2 bg-white/5 border border-white/[0.08] text-white rounded-lg text-sm focus:outline-none focus:border-blue-500/50"
 >
 ...
 <div className="max-h-48 overflow-y-auto bg-white/5 border border-white/[0.08] rounded-lg p-2 space-y-1">
 {groups.map(g => (
 ...
 className="w-4 h-4 rounded bg-white/[0.04] /10 border border-white/[0.08] text-emerald-500 focus:ring-emerald-500/50"
 />
 ...
 className="w-full px-3 py-2 bg-white/5 border border-white/[0.08] text-white placeholder:text-white/25 rounded-lg text-sm focus:outline-none focus:border-blue-500/50"
 />
 </div>
 <div>
 ...
 <th className="p-4 text-xs font-bold text-white/50 uppercase tracking-widest">Phase</th>
 <th className="p-4 text-xs font-bold text-white/50 uppercase tracking-widest">Group</th>
 <th className="p-4 text-xs font-bold text-white/50 uppercase tracking-widest">Date & Time</th>
 <th className="p-4 text-xs font-bold text-white/50 uppercase tracking-widest">Venue</th>
 </tr>
 </thead>
 <tbody>
 {isLoading ? (
 <tr><td colSpan={4} className="p-8 text-center text-white/50 text-sm">Loading schedules...</td></tr>
 ) : schedules.length === 0 ? (
 <tr><td colSpan={4} className="p-8 text-center text-white/50 text-sm">No schedules created yet.</td></tr>
 ) : (
 schedules.map(schedule => {
 const dateObj = new Date(schedule.presentation_time);
 return (
 <tr key={schedule.schedule_id} className="border-b border-white/[0.08][0.03] last:border-0 hover:bg-white/[0.04]">
 <td className="p-4">
 ...
 <td className="p-4 text-sm font-semibold text-white/50">{schedule.group_name}</td>
 <td className="p-4">
 <div className="flex flex-col gap-0.5">
 <span className="text-sm text-white/50">{dateObj.toLocaleDateString()}</span>
 <span className="text-xs text-white/50">{dateObj.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
 </div>
 </td>
 <td className="p-4 text-sm text-white/50 flex items-center gap-1.5 mt-2">

        ...
        <AppShell currentPage="/student/peer-evaluation">
            <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-white/40 text-sm ml-[52px]">Anonymously grade your group members' contributions</p>
                </div>
            </div>
            ...
            {!isLoading && !error && members.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {members.map(member => {
                        ...
                        return (
                            <div key={member.student_id} className="p-7 bg-white/[0.02] border border-white/[0.05] rounded-3xl relative overflow-hidden hover:bg-white/[0.04] hover:border-white/[0.1] hover:-translate-y-1 transition-all hover:shadow-2xl">
                                {isSubmitted && (
                                    <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-md z-10 flex flex-col items-center justify-center animate-in fade-in duration-500">
                                ...
                                <div className="flex items-center gap-4 mb-8">
                                    <div className="w-12 h-12 rounded-full bg-gradient-to-br from-slate-700 to-slate-800 border border-white/10 flex items-center justify-center shadow-inner">
                                        <Users size={20} className="text-white/60" />
                                    </div>
                                    <div>
                                        <p className="text-white font-black text-lg tracking-wide">PRN: {member.prn_no}</p>
                                        <p className="text-white/40 text-xs uppercase tracking-widest font-bold mt-0.5">Group Member</p>
                                    </div>
                                </div>
                                
                                <div className="space-y-6">
                                    <div>
                                        <label className="block text-xs font-bold text-white/40 uppercase tracking-widest mb-3">Contribution Score (1-5)</label>
                                    ...
                                    <div>
                                        <label className="block text-xs font-bold text-white/40 uppercase tracking-widest mb-3">Private Comments for Committee</label>
                                        <textarea
                                            ...
                                            className="w-full h-28 bg-white/5 border border-white/10 rounded-2xl p-4 text-sm text-white placeholder:text-white/25 focus:outline-none focus:border-amber-500/50 focus:bg-white/10 transition-all resize-none"
                                        />
                                    </div>

...
export const StudentGroups: React.FC = () => {
 ...
 const [activeTab, setActiveTab] = useState<'DETAILS' | 'CHAT' | 'MEETINGS'>('DETAILS');
 ...
 return 'bg-white/5 text-white/50 border-white/[0.08]';
 };
 ...
 <p className="text-white/50 text-sm ml-[52px]">Manage your project groups and team members</p>
 </div>
 <button
 ...
 <div className="p-4 border-b border-white/[0.08][0.06] flex items-center justify-between">
 <p className="text-xs font-bold text-white/50 uppercase tracking-widest">Groups ({groups.length})</p>
 {isLoading && <Loader2 size={13} className="animate-spin text-white/50" />}
 </div>
 {groups.length === 0 && !isLoading && (
 <div className="p-8 text-center text-white/50 text-xs font-semibold">
 ...
 className={`w-full text-left p-4 border-b border-white/[0.08][0.04] transition-all flex items-center justify-between group relative overflow-hidden ${
 ...
 <p className="text-xs text-white/50 mt-0.5">{group.member_count}/4 members</p>
 </div>
 ...
 <ChevronRight size={14} className="text-white/50 group-hover:text-white/50 transition-colors" />
 </div>
 </button>
 ))}
 </div>
 ...
 <Users size={36} className="text-white/50 mb-3" />
 <p className="text-sm text-white/50">Select a group to view details</p>
 </div>
 ) : (
 <div className="space-y-5">
 {/* Tabs */}
 <div className="flex gap-4 border-b border-white/[0.08] pb-2 mb-4">
 <button
 onClick={() => setActiveTab('DETAILS')}
 className={`text-sm font-bold pb-2 border-b-2 transition-colors ${activeTab === 'DETAILS' ? 'border-blue-500 text-blue-400' : 'border-transparent text-white/50 hover:text-white/50'}`}
 >
 Group Details
 </button>
 <button
 onClick={() => setActiveTab('CHAT')}
 className={`text-sm font-bold pb-2 border-b-2 transition-colors ${activeTab === 'CHAT' ? 'border-blue-500 text-blue-400' : 'border-transparent text-white/50 hover:text-white/50'}`}
 >
 Discussion
 </button>
 <button
 onClick={() => setActiveTab('MEETINGS')}
 className={`text-sm font-bold pb-2 border-b-2 transition-colors ${activeTab === 'MEETINGS' ? 'border-blue-500 text-blue-400' : 'border-transparent text-white/50 hover:text-white/50'}`}
 >
 Meetings
 </button>
 </div>
 ...
 className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-white/50 hover:text-white border border-white/[0.08] hover:border-white/[0.08] rounded-xl transition-all"
 >
 Lock & Submit
 </button>
 )}
 ...
 {selectedGroup.status !== 'FORMING' && selectedGroup.status !== 'WAITING_TOPIC_APPROVAL' ? (
    <button
      disabled
      className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold bg-white/10 text-white/50 rounded-xl cursor-not-allowed"
      title="Topic already approved"
    >
 ...
 {/* Members */}
 <div>
 <p className="text-[10px] font-bold text-white/50 uppercase tracking-widest mb-3">Team Members</p>
 {members.length === 0 ? (
 <p className="text-xs text-white/50">No members. Add members by their PRN.</p>
 ) : (
 <div className="grid grid-cols-2 gap-2">
 {members.map(m => (
 <div key={m.student_id} className="flex items-center justify-between p-3.5 rounded-2xl bg-white/[0.04] border border-white/[0.08][0.05] hover:bg-white/[0.04] hover:border-white/[0.08][0.1] transition-all hover:-translate-y-0.5 hover:shadow-lg">
 ...
 <p className="text-[10px] text-white/50">{m.prn_no}</p>
 </div>
 </div>
 {isLeader && m.student_id !== user?.user_id && selectedGroup.status === 'FORMING' && (
 <button 
 onClick={() => handleRemoveMember(m.student_id)}
 className="p-1.5 text-white/50 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors border border-transparent hover:border-red-500/20"
 title="Remove Member"
 >
 <UserMinus size={14} />
 </button>
 ...
 <div className="rounded-2xl bg-white/[0.04] border border-white/[0.08] p-5">
 <StudentMeetings groupId={selectedGroup.group_id} />
 </div>
 )}
 </div>
 )}
 ...
 <div className="absolute inset-0 bg-white/5 backdrop-blur-sm" onClick={() => setShowCreateModal(false)} />
 <div className="relative w-full max-w-md bg-white/[0.02] dark:bg-slate-900/95 backdrop-blur-xl border border-white/[0.08] rounded-3xl p-7 shadow-2xl">
 <h3 className="text-lg font-bold text-white mb-6">Create New Group</h3>
 <form onSubmit={handleCreateGroup} className="space-y-4">
 <div>
 <label className="block text-xs font-bold text-white/50 uppercase tracking-widest mb-2">Group Name</label>
 <input
 type="text"
 value={newGroupName}
 ...
 className="flex-1 py-2.5 text-sm text-white/50 border border-white/[0.08] rounded-xl hover:bg-white/5 font-semibold transition-all">Cancel</button>
 ...
 {showAddMemberModal && (
 <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
 <div className="absolute inset-0 bg-white/5 backdrop-blur-sm" onClick={() => setShowAddMemberModal(false)} />
 <div className="relative w-full max-w-md bg-white/[0.02] dark:bg-slate-900/95 backdrop-blur-xl border border-white/[0.08] rounded-3xl p-7 shadow-2xl">
 <h3 className="text-lg font-bold text-white mb-6">Add Team Member</h3>
 <form onSubmit={handleAddMember} className="space-y-4">
 <div>
 <label className="block text-xs font-bold text-white/50 uppercase tracking-widest mb-3">Select Students</label>
 ...
 isTaken ? 'bg-white/5 border-white/[0.08] opacity-50 cursor-not-allowed' : 
 isSelected ? 'bg-blue-500/10 border-blue-500/30' : 'bg-white/5 border-white/[0.08] hover:border-white/[0.08] hover:bg-white/[0.04]'
 }`}
 >
 ...
 className="w-4 h-4 rounded border-white/[0.08] bg-white/5 text-blue-500 focus:ring-0 focus:ring-offset-0 cursor-pointer disabled:cursor-not-allowed"
 />
 </div>
 ...
 <p className="text-[10px] text-white/50">PRN: {student.prn_no}</p>
 </div>
 {isTaken && (
 ...
 className="flex-1 px-4 py-2.5 text-sm font-bold text-white/50 hover:text-white bg-white/5 hover:bg-white/[0.04] /10 rounded-xl transition-all">
 Cancel
 </button>

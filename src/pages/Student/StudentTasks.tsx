...
export const StudentTasks: React.FC = () => {
 ...
 <p className="text-white/50 text-sm ml-[52px]">Manage your team's workflow</p>
 </div>
 {groupId && !isCreating && (
 <button 
 ...
 <div className="mb-8 bg-white/[0.04] border border-white/[0.08][0.05] p-5 rounded-2xl flex flex-col gap-3">
 ...
 {isCreating && (
 <div className="mb-8 p-6 bg-white/[0.02] dark:bg-slate-900/60 backdrop-blur-xl border border-white/[0.08] rounded-3xl animate-in fade-in slide-in-from-top-2 shadow-2xl">
 <div className="flex items-center justify-between mb-5">
 <h3 className="text-sm font-bold text-white tracking-wide">New Task</h3>
 <button onClick={() => setIsCreating(false)} className="text-white/50 hover:text-white hover:bg-white/[0.04] /10 p-1.5 rounded-lg transition-all">
 <X size={16} />
 </button>
 </div>
 ...
 className="flex-1 bg-white/5 border border-white/[0.08] rounded-xl px-4 py-3 text-sm text-white placeholder:text-white/25 focus:outline-none focus:border-blue-500/50 focus:bg-white/[0.04] /10 transition-all"
 autoFocus
 />
 <select
 value={newTaskAssignedTo}
 onChange={(e) => setNewTaskAssignedTo(e.target.value)}
 className="w-full md:w-48 bg-white/5 border border-white/[0.08] rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500/50 focus:bg-white/[0.04] /10 transition-all appearance-none cursor-pointer"
 >
 <option value="" className="bg-white/[0.02] dark:bg-slate-900">Unassigned</option>
 {members.map((m: any) => (
 <option key={m.student_id} value={m.student_id} className="bg-white/[0.02] dark:bg-slate-900">
 ...
 className={`p-4 bg-white/[0.04] border border-white/[0.08] rounded-2xl group hover:bg-white/[0.04] hover:border-white/[0.08] hover:shadow-lg transition-all relative cursor-grab active:cursor-grabbing hover:-translate-y-0.5 ${draggedTaskId === task.task_id ? 'opacity-50 border-blue-500/50 shadow-[0_0_15px_rgba(59,130,246,0.3)]' : ''}`}
 >
 <button 
 onClick={() => deleteTask(task.task_id)}
 className="absolute top-3 right-3 text-white/50 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all hover:bg-red-500/10 p-1.5 rounded-lg"
 >
 <X size={14} />
 </button>
 ...
 initial={{ opacity: 0 }} 
 animate={{ opacity: 1 }} 
 className="text-center py-10 text-white/50 text-sm font-semibold tracking-wider"
 >
 No tasks here
 </motion.div>
 )}

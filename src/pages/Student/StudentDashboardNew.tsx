...
export const StudentDashboardNew: React.FC = () => {
 ...
 const activeGroup = groups.find(g => g.status === 'ACTIVE' || g.status === 'WAITING_ALLOCATION');

 const now = new Date();
 const year = now.getFullYear();
 const phasesRaw = [
 { label: 'Phase 1: Planning', dueDate: (timelines || {})?.phase1 || 'Jan 31', sub: 'Planning' },
 { label: 'Phase 2: Development', dueDate: (timelines || {})?.phase2 || 'Mar 31', sub: 'Development' },
 { label: 'Phase 3: Testing', dueDate: (timelines || {})?.phase3 || 'May 31', sub: 'Testing' },
 { label: 'Final Submission', dueDate: (timelines || {})?.phase4 || 'Jun 30', sub: 'Submission' },
 ];

 let currentPhaseIndex = -1;
 for (let i = 0; i < phasesRaw.length; i++) {
 if (now <= new Date(`${phasesRaw[i].dueDate}, ${year}`)) {
 currentPhaseIndex = i;
 break;
 }
 }
 ...
 const phases: Phase[] = phasesRaw.map((p, i) => ({
 label: p.label,
 dueDate: p.dueDate,
 status: i < currentPhaseIndex ? 'completed' : i === currentPhaseIndex ? 'current' : 'upcoming'
 }));

 const statCards = [
 {
 label: 'Current Phase',
 value: `Phase ${currentPhaseIndex + 1}`,
 sub: phasesRaw[currentPhaseIndex].sub,
 icon: TrendingUp,
 ...
 {
 label: 'Guide Status',
 value: activeGroup?.guide_email ? 'Assigned' : 'Pending',
 ...
 return diffDays > 7;
 }, [activeGroup, recentLogbooks, now]);

 const completedPhasesCount = phases.filter(p => p.status === 'completed').length;
 const progressPercentage = phases.length > 0 ? (completedPhasesCount / phases.length) * 100 : 0;
 ...
 className="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/[0.04] /10 border border-white/[0.08] rounded-xl text-sm font-semibold text-white transition-all"
 >
 ...
 <div>
 <p className="text-[10px] font-bold text-white/50 uppercase tracking-widest mb-0.5">Student Identity</p>
 ...
 <div>
 <p className="text-[10px] text-white/50 uppercase tracking-widest">PRN Number</p>
 ...
 className="p-1 text-white/50 hover:text-blue-400 rounded transition-colors"
 title="Copy PRN"
 >
 ...
 {/* Roll No */}
 <div>
 <p className="text-[10px] text-white/50 uppercase tracking-widest">Roll No</p>
 ...
 {/* Batch */}
 <div>
 <p className="text-[10px] text-white/50 uppercase tracking-widest">Batch Year</p>
 <p className="text-sm font-bold text-white mt-0.5">
 {(user as any)?.batch_year || '—'}
 </p>
 </div>
 </div>
 </div>
 ...
 <div className="w-full h-3 bg-white/5 rounded-full overflow-hidden border border-white/[0.08]">
 <div 
 className="h-full bg-gradient-to-r from-emerald-500 to-teal-400 rounded-full transition-all duration-1000 ease-out" 
 style={{ width: `${progressPercentage}%` }} 
 />
 </div>
 <div className="flex justify-between mt-3 text-xs font-semibold text-white/50">
 ...
 <p className="text-[11px] font-medium text-white/50 mt-1 uppercase tracking-wider flex items-center gap-1.5">
 <Clock size={10} />
 Due {phase.dueDate}
 </p>
 </div>
 ...
 className="flex-1 w-full bg-white/5 border border-white/[0.08] rounded-xl p-4 text-white focus:outline-none focus:border-indigo-500/50 resize-none text-sm placeholder:text-white/25"
 />
 </div>
 </div>
 ...
 {activeGroup && (
 <div className="rounded-2xl bg-white/[0.04] border border-white/[0.08][0.06] overflow-hidden">
 ...
 <div className="bg-white/[0.04] border border-white/[0.08][0.05] rounded-xl p-4 text-center">
 <p className="text-2xl font-black text-white">{tasks.filter(t => t.status === 'TO_DO').length}</p>
 <p className="text-[10px] uppercase tracking-wider text-white/50 font-bold mt-1">To Do</p>
 </div>
 ...
 className="flex items-center gap-1.5 px-3 py-1.5 bg-white/5 hover:bg-white/[0.04] /10 border border-white/[0.08] text-white text-xs font-bold rounded-xl transition-all hover:-translate-y-0.5 active:translate-y-0"
 >
 <Plus size={13} />
 New Group
 </button>
 ...
 <div className="w-4 h-4 border-2 border-white/[0.08] border-t-purple-400 rounded-full animate-spin" />
 ...
 <p className="text-sm text-white/50 mb-2">No groups yet</p>
 <button 
 onClick={() => navigate('/student/groups', { state: { openCreateModal: true } })}
 ...
 : 'bg-white/[0.04] border-white/[0.08][0.05] hover:bg-white/[0.04]'
 }`}
 >
 {group.status === 'ACTIVE' && (
 ...
 {group.guide_email && (
 <>
 <span className="w-1 h-1 rounded-full bg-white/5/20" />
 ...
 : 'bg-white/5 text-white/50 border-white/[0.08]'
 }`}>
 ...
 <p className="text-xs text-white/50 text-center py-4">No recent logbooks.</p>
 )}
 {recentLogbooks.slice(0, 3).map((log) => (
 <div key={log.log_id} className="flex flex-col gap-2 p-3.5 rounded-xl bg-white/[0.04] border border-white/[0.08][0.05]">
 ...
 {log.guide_remarks && (
 <p className="text-xs font-medium text-white/50 bg-white/5 p-2 rounded-lg border border-white/[0.08]">
"{log.guide_remarks}"
 </p>
 )}
 </div>
 ...
 return (
 <div key={sched.schedule_id} className="p-4 rounded-xl bg-white/5 border border-white/[0.08]">

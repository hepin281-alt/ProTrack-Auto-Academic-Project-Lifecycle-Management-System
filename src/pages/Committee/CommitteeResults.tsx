...
export const CommitteeResults: React.FC = () => {
 ...
 return <span className="text-white/50 font-bold text-sm w-[18px] text-center">{index + 1}</span>;
 };
 ...
 <p className="text-white/50 text-sm ml-11">Leaderboard of evaluated project groups</p>
 </div>
 <div className="flex items-center gap-4 w-full md:w-auto flex-wrap">
 <select
 value={selectedPhase}
 onChange={(e) => setSelectedPhase(e.target.value)}
 className="px-3 py-2.5 bg-white/[0.04] border border-white/[0.08][0.1] text-white rounded-xl text-sm focus:outline-none focus:border-amber-500/50"
 >
 ...
 className="w-full pl-10 pr-4 py-2.5 bg-white/[0.04] border border-white/[0.08][0.1] text-white rounded-xl text-sm focus:outline-none focus:border-amber-500/50 transition-all"
 />
 </div>
 {isCoordinator && (
 ...
 onClick={handleLockAll}
 disabled={lockingPhase !== null}
 className="px-3 py-2.5 bg-white/5 text-white/50 border border-white/[0.08] font-bold rounded-xl flex items-center justify-center hover:bg-white/[0.04] /10 active:scale-95 disabled:opacity-50 transition-all"
 ...
 <th className="p-4 text-xs font-bold text-white/50 uppercase tracking-widest w-16 text-center">Rank</th>
 <th className="p-4 text-xs font-bold text-white/50 uppercase tracking-widest">Group Name</th>
 <th className="p-4 text-xs font-bold text-white/50 uppercase tracking-widest text-right">Score (/100)</th>
 <th className="p-4 text-xs font-bold text-white/50 uppercase tracking-widest text-center">Lock Status</th>
 <th className="p-4 text-xs font-bold text-white/50 uppercase tracking-widest text-right">Date Evaluated</th>
 <th className="p-4 text-xs font-bold text-white/50 uppercase tracking-widest text-center">Marksheet</th>
 {isCoordinator && <th className="p-4 text-xs font-bold text-white/50 uppercase tracking-widest text-center">Actions</th>}
 </tr>
 </thead>
 <tbody>
 {isLoading ? (
 <tr>
 <td colSpan={isCoordinator ? 6 : 5} className="p-8 text-center text-white/50 text-sm">
 Loading results...
 </td>
 </tr>
 ...
 <tr 
 key={result.eval_id} 
 className={`border-b border-white/[0.08][0.03] last:border-0 hover:bg-white/[0.04] transition-colors ${
 ...
 <p className="text-xs text-white/50 mt-0.5">Evaluated by: {result.evaluator_name}</p>
 )}
 </td>
 ...
 <p className="text-[10px] text-white/50">by {result.locked_by_name}</p>
 )}
 {result.locked_at && (

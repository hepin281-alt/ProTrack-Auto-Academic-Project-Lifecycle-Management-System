...
export const GuideReviews: React.FC = () => {
 ...
 <p className="text-white/50 text-sm ml-11">Review weekly submissions from your assigned groups</p>
 </div>
 <button
 onClick={fetchAllLogbooks}
 className="flex items-center gap-2 px-4 py-2.5 bg-white/[0.04] border border-white/[0.08][0.1] text-white/50 text-sm font-bold rounded-xl hover:bg-white/[0.04] /10 hover:text-white transition-all"
 >
 ...
 <div className="p-6 border-b border-white/[0.08][0.06] flex items-center justify-between bg-white/[0.04]">
 ...
 <CheckCircle2 size={48} className="text-white/50 mb-4" />
 <p className="text-sm text-white/50 font-semibold">All caught up! No pending logbooks.</p>
 </div>
 ) : (
 pendingLogs.map((log) => {
 const isSelected = selectedLogs.includes(log.log_id);
 return (
 <div key={log.log_id} className="group rounded-2xl border border-white/[0.08] overflow-hidden transition-all bg-white/[0.04] hover:bg-white/[0.04] hover:border-white/[0.08][0.15]">
 ...
 className="w-4 h-4 rounded border-white/[0.08] bg-white/5 text-blue-500 focus:ring-0 cursor-pointer"
 />
 </div>
 <button
 onClick={() => {
 ...
 <p className="text-xs text-amber-400/90 font-semibold">Week {log.week_number} <span className="text-white/50 mx-2">•</span> <span className="text-white/50">{new Date(log.created_at).toLocaleDateString()}</span></p>
 </div>
 ...
 <div className="p-6 border-b border-white/[0.08][0.06] flex items-center justify-between">
 <h2 className="text-sm font-bold text-white flex items-center gap-2">
 ...
 <div className="h-full flex flex-col items-center justify-center">
 <p className="text-sm text-white/50 font-semibold">No reviewed logbooks yet.</p>
 </div>
 ) : (
 reviewedLogs.map(log => (
 <div key={log.log_id} className="p-5 rounded-2xl border border-white/[0.08][0.05] bg-white/[0.04]">
 <div className="flex items-center justify-between mb-3">
 <p className="text-sm font-bold text-white/50">{log.group_name} <span className="text-xs text-white/50 font-normal ml-2">Week {log.week_number}</span></p>
 ...
 <p className="text-xs text-white/50 line-clamp-2 mb-3 leading-relaxed">{log.work_summary}</p>
 {log.guide_remarks && (
 <div className="bg-white/5 border border-white/[0.08][0.04] p-3 rounded-xl">
 ...
 {/* Panel */}
 <div className="relative w-full max-w-lg h-full bg-white/[0.02] dark:bg-slate-900 border-l border-white/[0.08] shadow-2xl flex flex-col animate-slide-up transform transition-transform duration-300">
 {/* Header */}
 <div className="p-6 border-b border-white/[0.08] bg-white/[0.04] flex items-center justify-between">
 <div>
 ...
 <button 
 onClick={() => setReviewPanel({ isOpen: false, log: null })}
 className="p-2 rounded-xl bg-white/5 hover:bg-white/[0.04] /10 text-white/50 hover:text-white transition-colors"
 >
 <X size={18} />
 </button>
 </div>
 ...
 {/* Summary */}
 <div>
 <h3 className="text-[10px] font-black text-white/50 uppercase tracking-widest mb-3">Work Summary</h3>
 <div className="p-5 rounded-2xl bg-white/5 border border-white/[0.08]">
 <p className="text-sm text-white/50 leading-relaxed whitespace-pre-wrap">{reviewPanel.log.work_summary}</p>
 </div>
 ...
 <div className="space-y-5 p-5 rounded-2xl bg-white/[0.04] border border-white/[0.08]">
 {[
 ...
 className="w-full h-1.5 bg-white/[0.04] /10 rounded-full appearance-none cursor-pointer accent-purple-500"
 />
 </div>
 ))}
 </div>
 ...
 {/* Remarks */}
 <div>
 <h3 className="text-[10px] font-black text-white/50 uppercase tracking-widest mb-3 flex items-center gap-2">
 ...
 className="w-full p-4 rounded-2xl bg-white/5 border border-white/[0.08] text-white placeholder:text-white/25 text-sm focus:outline-none focus:border-purple-500/50 transition-colors resize-none h-32"
 />
 </div>
 </div>

 {/* Footer Actions */}
 <div className="p-6 border-t border-white/[0.08] bg-white/[0.04] flex gap-3">
 <button 

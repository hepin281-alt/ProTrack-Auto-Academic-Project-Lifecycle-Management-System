...
import { MessageSquareDashed, Send, Loader2, AlertCircle } from 'lucide-react';
...
export const GroupChat: React.FC<GroupChatProps> = ({ groupId }) => {
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        setIsSending(true);
        try {
            await api.sendGroupMessage(token, groupId, newMessage.trim());
            setNewMessage('');
            await fetchMessages(); // Re-fetch to get exact DB timestamp/id
        } catch (err) {
    ...
    if (isLoading) {
        return (
            <div className="flex-1 flex flex-col items-center justify-center text-white/30 h-[400px]">
    ...
    return (
        <div className="flex flex-col h-full min-h-[400px] max-h-[600px]">
            ...
            <div className="flex-1 overflow-y-auto space-y-6 mb-4 pr-3 custom-scrollbar">
                {messages.length === 0 ? (
                    ...
                    messages.map((msg, idx) => {
                        ...
                        // Add spacing if previous message was from someone else
                                ...
                                <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} max-w-[75%]`}>
                                    {showAvatar && (
                                        <div className="flex items-baseline gap-2 mb-1.5 ml-1">
                                            <span className="text-[10px] font-bold text-white/50">{msg.sender_email.split('@')[0]}</span>
                                            <span className="text-[9px] text-white/30 uppercase tracking-wider">{msg.sender_role}</span>
                                        </div>
                                    )}
                                    ...
                                    <span className="text-[9px] font-semibold text-white/20 mt-1.5 mx-1">
            ...
            <form onSubmit={handleSendMessage} className="flex gap-3 mt-auto p-4 rounded-2xl bg-white/[0.02] border border-white/5 shadow-inner">
                <input
                    type="text"
                    value={newMessage}
                    ...
                    disabled={isSending}
                    className="flex-1 px-4 py-3 bg-white/[0.03] border border-white/10 text-white placeholder:text-white/30 rounded-xl text-sm focus:outline-none focus:border-purple-500/50 focus:bg-white/[0.05] transition-all disabled:opacity-50"
                />
                <button 
                    type="submit" 
